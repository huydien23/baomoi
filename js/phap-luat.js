// phap-luat.js
// Xử lý hiển thị dữ liệu cho trang Pháp luật
...
document.addEventListener("DOMContentLoaded", function () {
  console.log("Trang Pháp Luật đang tải...");
  ThemeManager.init();
  loadCategoryNews();
  setupAuthUI();
});
...
async function loadCategoryNews() {
  try {
    console.log("Đang tải tin tức trang Pháp luật...");

    // Tải tin tức chính chuyên mục Pháp luật
    const phapLuatNews = await fetchNewsFromRSS("phap-luat", 10);
    console.log("Tin pháp luật:", phapLuatNews);
    displayCategoryNews(".phap-luat-main .category-news-list", phapLuatNews);
    ...
    const kinhTeNews = await fetchNewsFromRSS("kinh-te", 3);
    displayCategoryNews(".kinh-te-news .category-news-list", kinhTeNews);
